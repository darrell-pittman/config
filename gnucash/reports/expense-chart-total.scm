...
(define-module (wgc report expense-chart-total))
...
(define menuname-expense (N_ "Expense (with total) Chart"))
...
(define menutip-expense
  (N_ "Shows a chart with my Expenses (with total) per interval \
developing over time"))
...
(define reportname-expense (N_ "Expense (with total) Over Time"))
...
Please deselect the accounts with negative balances."))
         ...
         (else
          (let* ((dates-list (if do-intervals?
            ...
            (set! col-totals (apply map + list-of-rows))
            ...
            (when show-table?
              (let ((table (gnc:make-html-table))
                ...
                (if rows>1? 
                  (gnc:html-table-append-row! table  (append (list (gnc:make-html-table-cell/markup "centered-label-cell" "Total"))

                                                             (map (lambda (amt)
                                                                    (make-monetary-cell amt)) col-totals)
                                                             (list (make-monetary-cell (apply + col-totals))))))
...
(for-each
 ...
 (list
  ...
  (list reportname-expense (list ACCT-TYPE-EXPENSE) #t menuname-expense menutip-expense #f "39849b247205442f9df0304dc6b8378f")
